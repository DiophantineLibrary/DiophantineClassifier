...
class Family:
    ...
    - ``constraints`` -- string; the side conditions under which the
      family's statements hold (domain of the unknowns, sign and size
      conditions on parameters), e.g. ``"x, y, p, q >= 2"`` for
      Catalan.  Empty when the bare equation over the domain is meant.
      Documentation for now: solvers state the hypotheses they use in
      their result's description rather than reading this field.
    ...
    EXAMPLES::
        ...
        True
        sage: family("linear").constraints
        ''
    """
    slug: str
    name: str
    ...
    def fill_code(self, data):
        ...
        EXAMPLES::
            ...
            sage: from diophantine_classifier import family
            sage: code = family("linear").fill_code({"coeffs": "3, 5",
            ....:                                       "b": 1})
            sage: "matrix(ZZ, [3, 5])" in code["sage"]
            True
            sage: "{coeffs}" in family("linear").fill_code({})["sage"]
            True
        """
        out = {}
    ...
    def formatted_references(self):
        ...
        EXAMPLES::
            ...
            sage: from diophantine_classifier import family
            sage: key, why, fmt = family(
            ....:     "univariate").formatted_references()[0]
            sage: key
            'Cohen2007'
            sage: "Cohen" in fmt
            True
...
def families():
    ...
    EXAMPLES::
        ...
        sage: fams = families()
        sage: "general-polynomial" in fams
        True
    ...
    for fname in sorted(os.listdir(DATA_DIR)):
        ...
        fam = Family(
            ...
            form=entry.get("form", ""),
            constraints=(entry.get("constraints") or "").strip(),
...
def depth(slug):
    ...
    EXAMPLES::
        ...
        0
        sage: depth("linear") > depth("general-polynomial")
        True
    """
    fam = families()[slug]
    if not fam.parents:
...
def ancestors(slug):
    ...
    This is a *set* of ancestors flattened into a list, not a chain: two
    consecutive entries need not be joined by an edge.  Use
    :func:`lineage_paths` or :func:`lineage_graph` to render ancestry, and
    this function only where a bare set of ancestors is wanted (deciding
    membership, walking candidate solvers).

    Ties at equal depth are broken alphabetically, so the order is stable
    across runs.
    ...
    OUTPUT: list of slugs, sorted by decreasing :func:`depth`, then by slug

    EXAMPLES::

        sage: from diophantine_classifier import ancestors
        sage: ancestors("linear")
        ['general-polynomial']
    ...
    The order is deterministic::
        ...
        sage: ancestors("linear") == ancestors("linear")
        True
    """
    seen = set()
    ...
    return sorted(seen, key=lambda s: (-depth(s), s))


def _paths_to_roots(slug, parents_of):
    r"""
    Every path from ``slug`` up to a root of a DAG.

    Pure graph traversal, with the edge relation passed in, so the path
    logic can be exercised on a small in-memory DAG without the registry.
    Paths are returned in sorted order and are therefore reproducible.

    INPUT:

    - ``slug`` -- the starting node
    - ``parents_of`` -- callable mapping a node to its parents

    OUTPUT: list of lists of nodes, each beginning with ``slug`` and ending
    at a node with no parents

    The graph is assumed acyclic (:func:`validate` enforces that for the
    registry); the visited-set guard only keeps a malformed one from
    recursing forever.

    EXAMPLES:

    A diamond has two paths, and neither of them joins the two middle
    nodes — which is exactly what a flattened ancestor list would suggest::

        sage: from diophantine_classifier.registry import _paths_to_roots
        sage: diamond = {"child": ("left", "right"), "left": ("root",),
        ....:            "right": ("root",), "root": ()}
        sage: _paths_to_roots("child", diamond.__getitem__)
        [['child', 'left', 'root'], ['child', 'right', 'root']]
        sage: _paths_to_roots("root", diamond.__getitem__)
        [['root']]
    """
    def walk(node, seen):
        parents = [p for p in dict.fromkeys(parents_of(node))
                   if p not in seen]
        if not parents:
            return [[node]]
        return sorted([node] + tail
                      for parent in parents
                      for tail in walk(parent, seen | {node}))

    return walk(slug, frozenset())


def lineage_paths(slug):
    r"""
    Every specialization path from ``slug`` up to a root family.

    A family with two parents yields two paths.  Unlike :func:`ancestors`,
    every consecutive pair here is a genuine DAG edge, so a path may be
    rendered as an arrow chain without inventing relationships.

    INPUT:

    - ``slug`` -- string; a family slug

    OUTPUT: list of lists of slugs, each starting with ``slug``; sorted, so
    repeated calls agree

    EXAMPLES::

        sage: from diophantine_classifier.registry import lineage_paths
        sage: lineage_paths("linear")
        [['linear', 'general-polynomial']]
        sage: lineage_paths("general-polynomial")
        [['general-polynomial']]
    """
    fams = families()
    return _paths_to_roots(slug, lambda s: fams[s].parents)


def lineage_graph(slug):
    r"""
    The ancestor subgraph above ``slug``: its nodes and their genuine edges.

    The serialization counterpart of :func:`lineage_paths`, for consumers
    that would rather draw the DAG than a list of chains.

    INPUT:

    - ``slug`` -- string; a family slug

    OUTPUT: dict with keys ``nodes`` (slugs, most specific first, ties
    alphabetical) and ``edges`` (``[child, parent]`` pairs, sorted)

    EXAMPLES::

        sage: from diophantine_classifier.registry import lineage_graph
        sage: g = lineage_graph("linear")
        sage: g["nodes"]
        ['linear', 'general-polynomial']
        sage: g["edges"]
        [['linear', 'general-polynomial']]
        sage: lineage_graph("general-polynomial")
        {'edges': [], 'nodes': ['general-polynomial']}
    """
    fams = families()
    nodes, edges = set(), set()
    frontier = [slug]
    while frontier:
        ...
        if node in nodes:
            continue
        nodes.add(node)
        for parent in fams[node].parents:
            edges.add((node, parent))
            frontier.append(parent)
    return {"nodes": sorted(nodes, key=lambda s: (-depth(s), s)),
            "edges": sorted([child, parent] for child, parent in edges)}
