...
def main(argv=None):
    ...
    JSON mode emits the ``as_dict`` payload::
        ...
        ....:     rc = main(["3*x + 5*y = 1", "--json"])
        sage: json.loads(buf.getvalue())["family"]
        'linear'

    Solutions keep their exact coordinates -- a rational root is a rational
    root, not the integer it truncates to::

        sage: buf = io.StringIO()
        sage: with contextlib.redirect_stdout(buf):
        ....:     rc = main(["2*x - 1 = 0", "--domain", "QQ", "--solve",
        ....:               "--json"])
        sage: json.loads(buf.getvalue())["solutions"]["solutions"]
        [['1/2']]
    """
    ...
    if args.json:
        ...
        if args.solve:
            try:
                # SolutionSet owns its serialization: coercing coordinates
                # through int() here used to turn the rational root 1/2
                # into 0
                out["solutions"] = solve(cls).as_dict()
