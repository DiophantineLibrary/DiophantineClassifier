...
from .transforms import CoordinateTransform, identity, negate, rename
...
class Match:
    ...
    - ``data`` -- dict of family-specific extracted data, with stringified
      ...
      for a Weierstrass match).
    - ``summary`` -- string; one-line human-readable description with the
      parameters filled in.
    - ``transform`` -- a
      :class:`~diophantine_classifier.transforms.CoordinateTransform`: the
      executable, invertible map from the user's variables to the family's
      standard coordinates, together with the structural roles the family
      assigns them.  The identity (with no operations) when the equation was
      already in standard form.  A solver works in the standard coordinates
      and calls ``transform.pull_back`` to state its answer in the user's.

    EXAMPLES::

        sage: from diophantine_classifier.matchers import run
        sage: from diophantine_classifier.parsing import parse
        sage: m = run(parse("3*x + 5*y = 1"))[-1]
        sage: m.slug, m.data["b"]
        ('linear', '1')
        sage: m.transform.pull_back({"x": 1766319049, "y": 226153980})
        {'x': 1766319049, 'y': 226153980}
    """
    slug: str
    data: dict = field(default_factory=dict)
    summary: str = ""
    transform: CoordinateTransform = field(
...
def _unit_coefficient(t):
    r"""
    Whether a term's *full* scalar coefficient is ``+1`` or ``-1``.

    A :class:`~diophantine_classifier.parsing.Term` keeps its parameter
    factors apart from ``coeff``, so ``A*2^n`` and ``2^n`` share
    ``coeff == 1``.  Named classical shapes — Pillai, Catalan, Fermat,
    Ramanujan–Nagell — are statements about unit multipliers and must test
    the whole coefficient, not just the rational part.

    INPUT:

    - ``t`` -- a :class:`~diophantine_classifier.parsing.Term`
    ...
    EXAMPLES::
        ...
        sage: from diophantine_classifier.parsing import parse
        sage: from diophantine_classifier.matchers import _unit_coefficient
        sage: [_unit_coefficient(t) for t in parse("2^n - 3^m = 1").terms]
        [True, True, True]
        sage: pe = parse("A*2^n - 3^m = 1", params="A")
        sage: [_unit_coefficient(t) for t in pe.exponential_terms()]
        [False, True]
    """
    return not t.param_powers and abs(t.coeff) == 1


def _concrete_coefficients(terms):
    r"""
    Whether every term in ``terms`` has a parameter-free coefficient.

    Guards the matchers that read coefficients off a term list as plain
    rationals (monicity, a fixed constant multiplier, a known sign); with a
    parameter present those readings drop the parameter silently.

    INPUT:

    - ``terms`` -- iterable of :class:`~diophantine_classifier.parsing.Term`
    ...
    EXAMPLES::
        ...
        sage: from diophantine_classifier.parsing import parse
        sage: from diophantine_classifier.matchers import _concrete_coefficients
        sage: _concrete_coefficients(parse("x^2 + 7 = 2^n").terms)
        True
        sage: _concrete_coefficients(parse("A*x^2 + 7 = 2^n", params="A").terms)
        False
    """
    return not any(t.param_powers for t in terms)
...
def _gram(P, R):
    ...
    Parametric coefficients stay in their own ring instead of being coerced
    to `QQ`, which a parameter cannot survive::

        sage: S.<D> = QQ[]
        sage: T.<x, y, z> = S[]
        sage: _gram(x^2 + y^2 - D*z^2, T)[2][2]
        -D
    """
    gens = R.gens()
    k = len(gens)
    base = R.base_ring()
    G = [[base.zero()] * k for _ in range(k)]
    for i in range(k):
        G[i][i] = base(P.monomial_coefficient(gens[i] ** 2))
        for j in range(i + 1, k):
            half = base(P.monomial_coefficient(gens[i] * gens[j])) / 2
            G[i][j] = G[j][i] = half
    return G
    ...
    if d == 2:
        ...
        if dd == 0 and ee == 0 and f0 != 0:
            ...
            if b == 0 and _sign_of(a) == 1 and _sign_of(c) == -1 \
                    and a != 1 and c == -1:
                # D x^2 - y^2 = N: read it as y^2 - D x^2 = -N, which is the
                # standard form with the variables swapped.  Negating the
                # right-hand side is part of the swap: dropping it turned
                # 5x^2 - y^2 = 1 into the different equation x^2 - 5y^2 = 1
                a, c, n = -c, -a, -n
                swapped = True
            src = (str(x), str(y))
            transform = rename(
                zip(("x", "y"), reversed(src) if swapped else src),
                description="swapped variables" if swapped else "",
    ...
    for v, u in orientations:
        ...
        if B1.degree(u) <= 1 and n == 3 \
            ...
            data = {"ainvs": "[%s]" % ", ".join(str(t) for t in ainvs),
                    ...
                    "x": str(u), "y": str(v)}
            curve_map = _curve_transform(u, v)
        ...
        if B1 == 0 and n == 4:
            ...
            return [Match(
                ...
                transform=_curve_transform(u, v),
...
def _curve_transform(u, v):
    r"""
    The coordinate map for a two-variable curve shape ``v^m = f(u)``.

    Both orientations of the pair are tried when matching, so the standard
    ``x`` (the base) and ``y`` (the power) may be either of the user's
    variables.  That reversal has to be executable, not merely recorded in
    the match data: a solver returns points ``(x, y)`` of the standard model
    and the map carries them back.

    INPUT:

    - ``u``, ``v`` -- the ring generators playing the standard ``x`` and ``y``
    ...
    EXAMPLES::
        ...
        sage: from diophantine_classifier.parsing import parse
        sage: from diophantine_classifier.matchers import _curve_transform
        sage: R = parse("y^2 = x^3 - 2").poly_ring
        sage: y, x = R.gens()                    # y appears first here
        sage: t = _curve_transform(x, y)
        sage: t.pull_back({"x": 3, "y": 5})
        {'x': 3, 'y': 5}
        sage: t.roles
        {'x': 'x', 'y': 'y'}
    """
    reversed_roles = str(u) != "x" or str(v) != "y"
    return rename(
        [("x", str(u)), ("y", str(v))],
        description=(f"read {u} as the base and {v} as the power"
                     if reversed_roles else ""))
    ...
    if scan:
        ...
        if len(entries) == k:
            ...
            if c0 == 0 and k == 3 and all(e >= 2 for e in exps) \
                ...
                names = pe.unknowns          # == the diagonal variables here
                transform = identity(names)
                if abs(sum(signs)) == 3:
                    ...
                    if flippable:
                        ...
                        transform = negate(
                            names, (entries[i][1],),
                            description=f"substituted {entries[i][1]} -> "
                ...
                if abs(sum(signs)) == 1:
                    ...
                    if sum(signs) == -1:
                        ...
                        signs = [-s for s in signs]
                        operations = ("multiplied by -1",)
                    # sorting by exponent reorders the variables, so carry
                    # each one's name along: a, p and standard x have to
                    # keep meaning the same term
                    quads = list(zip(coeffs, [v for _, v, _ in entries],
                                     exps, signs))
                    pos = sorted((e, c, v) for c, v, e, s in quads if s > 0)
                    neg = [(e, -c, v) for c, v, e, s in quads if s < 0]
                    (p, ca, xsrc), (q, cb, ysrc) = pos
                    (r, cc, zsrc) = neg[0]
                    transform = transform.then(rename(
            ...
            if c0 == 0 and k >= 4 and len(set(exps)) == 1 \
                ...
                # "equal sums of like powers" means the powers themselves are
                # summed: weighted forms such as 2*x^3 + y^3 = z^3 + 7*w^3 are
                # diagonal, not equal sums
                unit_coefficients = all(abs(c) == 1 for c in coeffs)
                if 0 < s_count and 0 < t_count and unit_coefficients:
                    out.append(Match(
...
def _match_exponential(pe):
    ...
    if not var_pow and not nonconst_poly and not mixed and pure_exp:
        ...
        if len(pure_exp) == 2 and single and not param_const and c0 != 0:
            ...
            # a^x - b^y = c is a statement about *unit* multipliers: A*2^n is
            # not 2^n, and t.coeff alone does not see the A
            if _unit_coefficient(t1) and _unit_coefficient(t2) and s1 == -s2 \
                ...
                out.append(Match(
                    ...
                    transform=rename([("x", xvar), ("y", yvar)]),
    ...
    if not var_pow and not mixed and len(pure_exp) == 1 and nonconst_poly \
        ...
        if len(polyvars) == 2 and len(degs) == 1:
            d = degs.pop()
            # m is a fixed constant multiplier: a parametric one is not it
            if d >= 3 and not pure_exp[0].param_powers:
                t = pure_exp[0]
                m = -t.coeff
                primes = [b for b, _ in t.exp_terms]
                form = " + ".join(
                    "%s*%s" % (tt.coefficient_string(),
                               "*".join(f"{v}^{e}" for v, e in tt.powers))
    ...
    if len(simple_vv) == len(var_pow) and var_pow and not pure_exp \
        ...
        if len(var_pow) == 2 and not nonconst_poly and not param_const:
            ...
            if _unit_coefficient(t1) and _unit_coefficient(t2) and s1 == -s2 \
                ...
                if xb != yb and pv != qv and abs(c) == 1:
                    ...
                    out.append(Match(
                        "catalan",
                        transform=rename([("x", xb), ("p", pv), ("y", yb),
                                          ("q", qv)],
                                         description="oriented as x^p - y^q = 1"
                                         if c == -1 else ""),
        ...
        if len(var_pow) == 3 and not nonconst_poly and c0 == 0 \
            ...
            if all(_unit_coefficient(t) for t in var_pow) and None not in signs \
                    and abs(sum(signs)) == 1 and len(set(bases)) == 3:
                # the exponents are themselves unknowns, and x^p + y^p = z^q
                # reuses one of them.  A rename onto independent canonical
                # p, q, r would claim a bijection that does not exist, so
                # the coordinate map stays the identity and the structure is
                # recorded as roles instead
                roles = {"bases": list(bases), "exponents": list(expvars)}
                if len(set(expvars)) == 1:
                    out.append(Match(
                        "fermat", data={"n": expvars[0], "symbolic": True},
                        transform=identity(pe.unknowns, roles=roles),
                        summary=f"Fermat equation with unknown exponent "
                                f"{expvars[0]} (no solutions for "
                                f"{expvars[0]} >= 3, Wiles)",
                    ))
                    return out
                # any other symbolic signature is generalized Fermat: the
                # exponents need not be pairwise distinct, so x^p + y^p = z^q
                # belongs here just as x^p + y^q = z^r does
                out.append(Match(
                    "generalized-fermat",
        ...
        if len(var_pow) == 1 and nonconst_poly and not param_const:
            ...
            # f(u) is read off as plain rational coefficients below (its
            # leading one has to be 1 for Lebesgue-Nagell), so a parametric
            # coefficient anywhere would be dropped silently
            if len(polyvars) == 1 and polyvars[0] != base \
                    and _unit_coefficient(t) \
                    and _concrete_coefficients(poly_terms):
                u = polyvars[0]
    ...
    if not var_pow and not mixed and len(pure_exp) == 1 and nonconst_poly:
        polyvars = sorted({v for t in nonconst_poly for v, _ in t.powers})
        # the multiplier k and the quadratic's coefficients (monicity below)
        # are read as plain rationals: a parametric factor would vanish, and
        # A*x^2 + 7 = 2^n is not a Ramanujan-Nagell equation
        if len(polyvars) == 1 and not pure_exp[0].param_powers \
                and _concrete_coefficients(poly_terms):
            u = polyvars[0]
            t = pure_exp[0]
            k_coeff = -t.coeff
...
def run(pe):
    ...
    Every emitted match carries a coordinate map; a matcher that found the
    equation already in standard form gets the identity on the equation's
    own unknowns, so a solver can always call ``pull_back``::

        sage: from diophantine_classifier.parsing import parse
        sage: from diophantine_classifier.matchers import run
        sage: m = run(parse("3*x + 5*y = 1"))[-1]
        sage: m.transform.is_identity, m.transform.source_variables
        (True, ('x', 'y'))
    ...
    for m in matches:
        if m.slug in seen:
            continue
        seen.add(m.slug)
        if not m.transform.source_variables:
            # a matcher that normalized nothing still owes the solvers a
            # map: the identity on this equation's unknowns
            m.transform = identity(pe.unknowns,
                                   description=m.transform.description,
                                   operations=m.transform.operations,
                                   roles=m.transform.roles)
        result.append(m)
    return result
